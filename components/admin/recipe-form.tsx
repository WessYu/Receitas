...
export function RecipeForm({
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <section className="rounded-lg border border-border bg-surface/85 p-6 shadow-sm">
        <div className="grid gap-5 md:grid-cols-2">
          <div className="md:col-span-2">
            <label className="mb-2 block text-sm font-semibold text-ink" htmlFor="title">
              Título
            </label>
          ...
          <div className="md:col-span-2">
            <label className="mb-2 block text-sm font-semibold text-ink" htmlFor="imageUrl">
              Imagem por URL
            </label>
          ...
          <div className="md:col-span-2">
            <label className="mb-2 block text-sm font-semibold text-ink" htmlFor="imageFile">
              Enviar foto
            </label>
            <input
              className="field file:mr-4 file:rounded-md file:border-0 file:bg-olive file:px-4 file:py-2 file:text-sm file:font-semibold file:text-background"
              id="imageFile"
              name="imageFile"
              type="file"
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-ink" htmlFor="categoryId">
              Categoria
            </label>
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-ink" htmlFor="difficulty">
              Dificuldade
            </label>
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-ink" htmlFor="prepTime">
              Tempo de preparo
            </label>
            <input className="field" id="prepTime" name="prepTime" type="number" min="1" defaultValue={recipe?.prepTime ?? 30} />
          </div>
          <div>
            <label className="mb-2 block text-sm font-semibold text-ink" htmlFor="servings">
              Porções
            </label>
          ...
          {mode === "admin" ? (
            <>
              <label className="flex items-center gap-3 rounded-md border border-border bg-elevated px-4 py-3 text-sm font-semibold text-ink">
                <input className="accent-olive" type="checkbox" name="published" defaultChecked={recipe?.published ?? true} />
                Publicada
              </label>
              <label className="flex items-center gap-3 rounded-md border border-border bg-elevated px-4 py-3 text-sm font-semibold text-ink">
                <input className="accent-olive" type="checkbox" name="featured" defaultChecked={recipe?.featured ?? false} />
                Destaque
              </label>
            </>
          ) : (
            <p className="rounded-md border border-olive/20 bg-olive/10 px-4 py-3 text-sm font-medium text-ink md:col-span-2">
      ...
      <section className="rounded-lg border border-border bg-surface/85 p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h2 className="font-serif text-3xl text-ink">Modo de preparo</h2>
